...
#'
#' @param dsmp a named list with genetic data for all the genotyped samples.
#'   Each element represents a sample and is itself a list with elements
#'   corresponding to loci.
#' @param dfppl a data frame with rows corresponding to individuals in the
...
#' @param dfsmp a data frame with rows corresponding to genotyped samples. The
...
#'   probability).
#' @param coi a vector of COI for each sample (the length and order should
#'   correspond to the samples in \code{dfsmp}).
#' @param pprior prior probability of recrudescence.
#' @param update_est a logical value indicating if the failure rate estimate
...
asterKM <- function(dsmp, dfppl, dfsmp, coi, afreq, rbg = 0, pprior = 0.5,
  ...
  for (i in irecur) {
    ...
    dfppl$logA0[i]  <- a0a1[[2]][1]
    dfppl$logA1[i]  <- a0a1[[2]][2]
  }

  est <- estKM(dfppl)
  dfppl$ppost_user <- Ppost(dfppl$logA0, dfppl$logA1, pprior)
  dfppl$ppost_empB <- Ppost(dfppl$logA0, dfppl$logA1, est)
...
Ppost <- function(a0, a1, pprior) {
  amax <- pmax(a0, a1)
  a1t  <- a1 + log(pprior)
  a0t1 <- a0 + log(1 - pprior)
  amax <- pmax(a1t, a0t1)
  denom <- amax + log(exp(a1t - amax) + exp(a0t1 - amax))
  return(exp(a1t - denom))
}
