...
Package `asterTES` performs recurrence classification and failure rate estimation of therapeutic efficacy studies (TES). To install it from [GitHub](https://github.com/):
...
Consider TES data that are stored in three data frames: `dfppl` (where rows represent individuals), `dfsmp` (rows represent genotyped samples), and `dfgen` (genotyping data, rows represent alleles).
...
To obtain individual recurrence classification and conditional probabilities of genotyping data given recrudescence, a pair of samples for the same individual is needed - the Day 0 sample and the recurrent sample. As an example, we classify recurrence from ID `A3`:
